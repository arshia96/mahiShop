...
def aboutus(request): 
    return render(request, 'blog/partials/aboutus.html')
...
def productDetail(request, pk, slug):
    ...
    _products = Product.objects.filter(tags__in=product_tags, status='published').exclude(id=product.id)
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                messages.info(request, "کدتخفیف نامعتبر است")
...
def checkout(request):
    if request.user.is_authenticated:
        ...
        if not (customer.first_name and customer.last_name and customer.address and customer.email and customer.phone):
            messages.error(request, 'لطفا مشخصات فردی خود را در پروفایل ثبت کنید')
...
class loginPage(LoginView):
    ...
    def form_invalid(self, form):
        messages.error(self.request, 'نام کاربری یا رمز عبور نامعتبر است')
...
def post_search(request):
    if 'query' in request.GET:
        ...
        lookup = Q(tags__name__icontains=query)
